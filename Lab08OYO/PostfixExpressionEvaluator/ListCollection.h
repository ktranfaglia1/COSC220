...
private:
    ...
    ListNode *head = nullptr; // Head pointer to keep track of linked list start
...
ListCollection<T>::ListCollection(const ListCollection &obj) {
    ListNode *nodePtr = obj.head; // Traversing varaiable to go through the list, pointer set to list header of object
...
int ListCollection<T>::size() {
    ...
    ListNode *nodePtr = head; // Traverssing pointer set to head
...
void ListCollection<T>::setElement(int element, T value) {   
    ...
    if (isValid(element)) {
        ListNode *nodePtr = head; // Traversing pointer set to head of list
...
template<class T>
T ListCollection<T>::getElement(int position) {
...
void ListCollection<T>::pushBack(T element) {
    ListNode *newNode = new ListNode(element); // Creates a new node pointer of with a value of element
...
T ListCollection<T>::popBack() {
    ...
    ListNode *nodePtr = head; // Traversing pointer set to head of list
    ListNode *previousNode = nullptr; // Pointer to keep track of previous node
    ...
    // Pointer control statment. Either sets head to nullptr or the last node to nullptr since the link is being deleted. Otherwise, it would point to something non-existent
    if (previousNode)
...
T ListCollection<T>::popFront() {
    ...
    ListNode *nodePtr = head; // Sets a node pointer to head
...
template<class T>
void ListCollection<T>::insertOrdred(T element) {
    ListNode *newNode = new ListNode(element); // Creates a new node pointer of with a value of element

    ListNode *nodePtr = head; // Traversing pointer set to head of list
    ListNode *previousNode = nullptr; // Pointer to track previous node
...
template<class T>
void ListCollection<T>::removeElement(T element) {
    ListNode *nodePtr = head; // Traversing pointer set to head of list
    ListNode *previousNode = nullptr; // Pointer to track previous node
    ...
    if (!nodePtr)
        ...
        previousNode->next = nodePtr->next; // A match was found for a node deletion so previos node is linked to the node after nodeptr ...  nodeptr's link
